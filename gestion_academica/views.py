...
from .utils import calcular_estado_academico_curso, obtener_desempeno, analizar_riesgo_academico_curso, generar_boletin_pdf_en_memoria, crear_lecciones_diarias_desde_planeacion
import pandas as pd 
import datetime
...
def inicio_academico(request):
    ...
    # 2. Acceso para staff/admin (Tu lógica original se mantiene)
    if user.is_staff:
        context = {
            'nombre_usuario': user.get_full_name() or user.username,
            'rol_usuario': getattr(user, 'rol', 'Administrador'),
            'mensaje': 'Bienvenido al Panel de Administración del Módulo Académico.',
        }
...
class GradoListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    model = Grado; template_name = 'gestion_academica/grado_lista.html'; context_object_name = 'grados'; permission_required = 'gestion_academica.view_grado'; paginate_by = 10
...
def portal_familiar_inicio(request):
    ...
    estudiantes_a_cargo = familiar_actual.estudiantes_asociados.select_related(
        'usuario', 
        'grado_actual'
...
def familiar_ver_calificaciones_estudiante(request, estudiante_pk):
    ...
    if periodo_activo and estudiante_seleccionado.grado_actual:
        # --- LÓGICA CORREGIDA ---
        # Esta es la consulta correcta y robusta para obtener los cursos.
        cursos_del_estudiante = Curso.objects.filter(
...
def docente_libro_de_notas_por_curso(request, curso_pk):
    # --- Tu lógica de seguridad y obtención de objetos se mantiene intacta ---
    ...
    if not curso.docentes_asignados.filter(pk=request.user.docente.pk).exists() and not request.user.is_superuser:
    ...
    context = {
        ...
        'libro_notas_data': libro_notas_data, # Pasamos los datos calculados
...
def detalle_curso_aula_virtual(request, curso_pk):
    ...
    elif usuario.is_staff and hasattr(usuario, 'institucioneducativa'):
        if usuario.institucioneducativa == institucion_curso:
            acceso_permitido = True
...
def dashboard_estudiante(request):
    ...
    # --- LÓGICA FINANCIERA (se mantiene intacta) ---
    ...
    cuentas_vencidas_count = cuentas_no_pagadas_qs.filter(fecha_vencimiento_especifica__lt=hoy).count()
    ...
    estudiante_moroso = cuentas_vencidas_count > 0

    # --- LÓGICA DE NOTIFICACIONES Y PERIODO (se mantiene intacta) ---
    notificaciones_sin_leer = Notificacion.objects.filter(destinatario=request.user, leido=False)
    ...
    if periodo_activo and estudiante.grado_actual:
        ...
        for act in ActividadCalificable.objects.filter(curso__in=cursos_del_estudiante, fecha_entrega_limite__range=(hoy, limite_agenda)).prefetch_related('preguntas'):
            url = reverse('gestion_academica:resolver_actividad', args=[act.pk]) if act.preguntas.exists() else (act.material_adjunto.url if act.material_adjunto else '#')
            eventos_agenda.append({'fecha': act.fecha_entrega_limite, 'titulo': f"Actividad: {act.titulo}", 'tipo': 'evaluacion', 'url': url, 'target_blank': bool(act.material_adjunto and not act.preguntas.exists())})
...
class DetalleActividadAPIView(RetrieveAPIView):
    ...
    def get_queryset(self):
        """
        Sobrescribimos get_queryset para optimizar la consulta a la base de datos.
        """
        return ActividadCalificable.objects.prefetch_related(
            Prefetch(
                'preguntas', 
                queryset=Pregunta.objects.order_by('orden').prefetch_related(
                    Prefetch('opciones', queryset=Opcion.objects.order_by('?')) # '?' para orden aleatorio
                )
            )
        )
# =========
...
def resolver_actividad_page(request, actividad_pk):
    ...
    # Validar institución
    if actividad.institucion != institucion:
        messages.error(request, "No tienes permiso para esta actividad.")
        return redirect('gestion_academica:dashboard_estudiante')

    # Verificar si ya hay intento en progreso
    ...
    # Si hay intento y está vencido, cerrarlo
    if intento and actividad.duracion_minutos:
        ...
        if tiempo_transcurrido.total_seconds() >= actividad.duracion_minutos * 60:
            ...
            intento.save()
            intento = None  # Eliminar la referencia para crear otro si aplica

    # Contar intentos válidos (opcional: puedes excluir tiempo_agotado si quieres)
    intentos_validos = IntentoActividad.objects.filter(
        estudiante=estudiante,
        actividad=actividad,
        institucion=institucion
    ).count()

    if intentos_validos >= actividad.numero_intentos_permitidos:
        messages.error(
            request, f"Has alcanzado el número máximo de {actividad.numero_intentos_permitidos} intentos para esta actividad."
        )
        return redirect('gestion_academica:dashboard_estudiante')

    # Si no hay intento válido en progreso, crear uno nuevo
    if not intento:
    ...
    print("INTENTO:", intento.pk, intento.estado, intento.inicio)
    if actividad.duracion_minutos:
        ...
        if tiempo_restante_segundos <= 0:
            ...
            intento.fin = timezone.now()
            intento.save()
            messages.error(request, f"Se ha agotado el tiempo para la actividad '{actividad.titulo}'.")
            return redirect('gestion_academica:dashboard_estudiante')

    # Mostrar actividad
    return render(request, 'gestion_academica/resolver_actividad.html', {
        'actividad': actividad,
...
class EnviarRespuestasAPIView(APIView):
    ...
    def post(self, request, actividad_pk, *args, **kwargs):
        ...
        try:
            intento = IntentoActividad.objects.get(
                estudiante=estudiante,
                actividad_id=actividad_pk,
                estado='en_progreso'
            )
        except IntentoActividad.DoesNotExist:
...
def resolver_actividad_page(request, actividad_pk):
    ...
    # --- LÓGICA DE BLOQUEO Y TEMPORIZADOR ---
    intento, created = IntentoActividad.objects.get_or_create(
        estudiante=estudiante,
        actividad=actividad,
        defaults={'estado': 'en_progreso'}
    )
    
    # Si el intento ya estaba completado, no se puede volver a entrar
    if not created and intento.estado == 'completado':
        messages.warning(request, f"Ya has completado la actividad '{actividad.titulo}'. No puedes volver a realizarla.")
        return redirect('gestion_academica:dashboard_estudiante')

    # Calculamos el tiempo restante si la actividad tiene duración
    tiempo_restante_segundos = None
    if actividad.duracion_minutos:
        tiempo_transcurrido = timezone.now() - intento.inicio
        tiempo_total_segundos = actividad.duracion_minutos * 60
    ...
    return render(request, 'gestion_academica/resolver_actividad.html', context)        
...
def dashboard_docente(request):
    ...
    if periodo_activo:
        ...
        if direccion_grupo:
            estudiantes_del_grupo = Estudiante.objects.filter(grado_actual=direccion_grupo.grado)
            ...
            for estudiante in estudiantes_del_grupo:
                for curso in cursos_del_grado:
                    ...
                    if resultado_riesgo['estado'] in ["En Riesgo", "Situación Crítica"]:
                        count_riesgo += 1
            context['cantidad_estudiantes_riesgo'] = count_riesgo
...
def link_callback(uri, rel):
    ...
    de archivos absoluta que xhtml2pdf pueda encontrar. VERSIÓN SIMPLIFICADA.
    """
    # Para archivos de MEDIA (logos de la institución, etc.)
    if uri.startswith(settings.MEDIA_URL):
        # une la raíz de MEDIA con el resto de la ruta del archivo
        path = os.path.join(settings.MEDIA_ROOT, uri.replace(settings.MEDIA_URL, "", 1))
    
    # Para archivos STATIC (marco del diploma, sello, etc.)
    elif uri.startswith(settings.STATIC_URL):
        # une el directorio de STATIC con el resto de la ruta del archivo
        # NOTA: Esto asume que tienes una sola carpeta en STATICFILES_DIRS
        path = os.path.join(settings.STATICFILES_DIRS[0], uri.replace(settings.STATIC_URL, "", 1))
        
    else:
        # Si no es una URL de media o static, no la toques
        return uri

    # Comprobar si el archivo realmente existe antes de devolverlo
    if not os.path.isfile(path):
        print(f"ADVERTENCIA: No se encontró el archivo en la ruta: {path}")
        return None
...
def generar_mencion_pdf(request, mencion_pk):
    ...
    if not (es_el_mismo_estudiante or es_familiar_asociado or request.user.is_staff):
        messages.error(request, "No tienes permiso para ver este reconocimiento.")
        return redirect('gestion_academica:inicio_academico')
...
def exportar_observador_pdf(request, estudiante_pk):
    """
    Genera un informe en PDF con todas las anotaciones del observador de un estudiante.
    """
    estudiante = get_object_or_404(Estudiante.objects.select_related('usuario', 'grado_actual', 'institucion'), pk=estudiante_pk)
    anotaciones = AnotacionObservador.objects.filter(estudiante=estudiante).select_related('registrado_por', 'curso__materia')
    ...
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'inline; filename="Observador_{estudiante}.pdf"'
...
class DocenteTipoActividadCreateView(LoginRequiredMixin, CreateView):
    model = TipoActividad
    form_class = TipoActividadForm
    template_name = 'gestion_academica/docente_tipo_actividad_formulario.html'
    success_url = reverse_lazy('gestion_academica:docente_lista_tipos_actividad')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titulo_formulario'] = "Crear Nueva Categoría"
        return context

    def form_valid(self, form):
        """Asigna automáticamente el autor y la institución antes de guardar."""
        form.instance.creado_por = self.request.user
        form.instance.institucion = self.request.user.institucion_asociada
        messages.success(self.request, "Categoría de calificación creada exitosamente.")
        return super().form_valid(form)


class DocenteTipoActividadUpdateView(LoginRequiredMixin, UpdateView):
    model = TipoActividad
    form_class = TipoActividadForm
    template_name = 'gestion_academica/docente_tipo_actividad_formulario.html'
    success_url = reverse_lazy('gestion_academica:docente_lista_tipos_actividad')
    ...
    def get_queryset(self):
        """
        Seguridad Mejorada: Un docente puede editar una categoría si:
        1. Él mismo la creó.
        2. La categoría es del sistema (creado_por es NULO).
        """
        return TipoActividad.objects.filter(
            Q(creado_por=self.request.user) | Q(creado_por__isnull=True),
            institucion=self.request.user.institucion_asociada
        )

    def form_valid(self, form):
        """
        Al guardar, si la categoría era del sistema, se asigna al docente actual.
        """
        # Si la categoría no tenía un autor, el docente que la edita ahora es el "dueño".
        if not self.object.creado_por:
            form.instance.creado_por = self.request.user
        messages.success(self.request, "Categoría actualizada exitosamente.")
        return super().form_valid(form)

    # El método get_context_data se mantiene igual, no necesita cambios.
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titulo_formulario'] = "Editar Categoría"
        return context


class DocenteTipoActividadDeleteView(LoginRequiredMixin, DeleteView):
    model = TipoActividad
    template_name = 'gestion_academica/docente_tipo_actividad_confirmar_eliminar.html'
    success_url = reverse_lazy('gestion_academica:docente_lista_tipos_actividad')
    context_object_name = 'categoria'

    def get_queryset(self):
        """Seguridad: Un docente solo puede eliminar las categorías que él creó."""
        return TipoActividad.objects.filter(creado_por=self.request.user)    
...
def dashboard_coordinador_view(request):
    ...
    context = {
        ...
        'porcentaje_asistencia_hoy': porcentaje_asistencia_hoy, # <-- Pasamos el nuevo dato
...
def dashboard_bienestar_view(request):
    """
    Muestra un dashboard confidencial con todas las observaciones
    que la IA ha marcado para revisión.
    """
    # Lógica de permisos: solo los roles designados pueden ver esto.
    if not (request.user.is_superuser or (request.user.is_staff and request.user.rol in ['coordinador', 'psicologo'])):
        messages.error(request, "Acceso denegado. Esta sección es confidencial.")
        return redirect('gestion_academica:inicio_academico')

    # Buscamos todas las anotaciones marcadas para revisión
    alertas_sentimiento = AnotacionObservador.objects.filter(
        requiere_revision=True
    ).select_related(
        'estudiante__usuario', 'registrado_por'
    ).order_by('-fecha_hora')

    resumenes_ia = AnalisisComportamientoIA.objects.filter(
        institucion=request.user.institucion_asociada
    ).select_related('estudiante__usuario').order_by('-fecha_analisis')

    context = {
        'titulo_pagina': "HALU Sentinel - Dashboard de Bienestar",
        'alertas': alertas_sentimiento,
        'resumenes_ia': resumenes_ia # <-- Pasamos los nuevos datos a la plantilla
    }
...
def detalle_alerta_bienestar_view(request, pk):
    """
    Muestra el detalle completo de una única anotación marcada para revisión.
    """
    # Lógica de permisos
    if not (request.user.is_superuser or (request.user.is_staff and request.user.rol in ['coordinador', 'psicologo'])):
    ...
    # Obtenemos la anotación específica con datos relacionados para eficiencia
    alerta = get_object_or_404(
        AnotacionObservador.objects.select_related(
            'estudiante__usuario', 'registrado_por'
        ), 
        pk=pk,
        requiere_revision=True # Doble seguridad: solo se pueden ver las marcadas para revisión
    )
...
def familiar_seleccionar_docente_cita(request):
    ...
    cursos_de_los_hijos = Curso.objects.filter(grado__in=[e.grado_actual for e in estudiantes_asociados])
...
def familiar_agendar_cita_view(request, docente_pk):
    ...
    # --- INICIO DE LA LÓGICA DE BLOQUEO DE HORARIOS ---

    # 1. Obtenemos todas las citas futuras que ya han sido agendadas con este docente
    #    y que no están canceladas.
    citas_existentes = CitaReunion.objects.filter(
        docente=docente,
        fecha_hora_inicio__gte=timezone.now()
    ).exclude(estado='CANCELADA')

    # 2. Creamos un conjunto (set) con los horarios ya ocupados para una búsqueda súper rápida.
    slots_ocupados = {cita.fecha_hora_inicio for cita in citas_existentes}

    # 3. Generamos los slots de tiempo disponibles basándonos en la disponibilidad del docente.
    disponibilidades = DisponibilidadDocente.objects.filter(docente=docente)
    slots_disponibles = []
    hoy = timezone.localdate()
    # Generamos slots para los próximos 14 días.
    for i in range(14): 
        fecha_actual = hoy + timedelta(days=i)
        dia_semana_actual = fecha_actual.weekday() # Lunes=0, Martes=1...
        
        # Buscamos si el docente tiene disponibilidad para este día de la semana.
        for disp in disponibilidades.filter(dia_semana=dia_semana_actual):
            # Creamos objetos de fecha y hora completos y conscientes de la zona horaria.
            hora_actual_slot = timezone.make_aware(datetime.combine(fecha_actual, disp.hora_inicio))
            hora_fin_bloque = timezone.make_aware(datetime.combine(fecha_actual, disp.hora_fin))
            
            # Generamos slots de 15 minutos dentro del bloque de disponibilidad.
            while hora_actual_slot < hora_fin_bloque:
                # 4. ¡AQUÍ ESTÁ LA CLAVE! Solo añadimos el slot si NO está en el conjunto de ocupados.
                if hora_actual_slot not in slots_ocupados:
    ...
    if request.method == 'POST':
        ...
        # Convertimos la fecha del formulario a un objeto datetime consciente de la zona horaria.
        fecha_hora_cita = datetime.fromisoformat(fecha_hora_str)
        estudiante = get_object_or_404(Estudiante, pk=estudiante_id)

        # 5. Doble verificación de seguridad antes de guardar, por si dos personas reservan al mismo tiempo.
        if CitaReunion.objects.filter(docente=docente, fecha_hora_inicio=fecha_hora_cita).exists():
            messages.error(request, "Lo sentimos, ese horario acaba de ser reservado. Por favor, elige otro.")
            return redirect('gestion_academica:familiar_agendar_cita', docente_pk=docente.pk)
        ...
        messages.success(request, f"Cita con {docente.usuario.get_full_name()} agendada exitosamente.")
        return redirect('gestion_academica:portal_familiar_inicio')
...
def dashboard_docente_api_view(request):
    ...
    try:
        ...
        cantidad_riesgo = 0
        if es_director:
            estudiantes_grupo = Estudiante.objects.filter(grado_actual=direccion_grupo.grado)
            cursos_grado = Curso.objects.filter(grado=direccion_grupo.grado, periodo_academico=periodo_activo)
            for est in estudiantes_grupo:
                # Simulamos un conteo rápido para el widget
                if analizar_riesgo_academico_curso(cursos_grado.first(), est)['estado'] != 'Normal':
                    cantidad_riesgo += 1
...
def dashboard_estudiante_api_view(request):
    ...
    try:
        ...
        # 1. Widgets y Alertas
        notificaciones_sin_leer = Notificacion.objects.filter(destinatario=user, leido=False)
        ...
        hoy = timezone.now().date()
        limite_agenda = hoy + timedelta(days=14)
        cursos_del_estudiante = Curso.objects.filter(grado=estudiante.grado_actual, periodo_academico=periodo_activo)
...
@api_view(['GET'])
# No necesitamos @permission_classes porque la lógica interna ya valida al usuario
...
@api_view(['GET'])
# No es necesario @permission_classes si ya está en settings.py
...
def dashboard_eleccion_ia(request, eleccion_id):
    ...
    # --- INICIO DE LA CORRECCIÓN CLAVE ---
    # 1. Optimizamos la consulta para evitar múltiples llamadas a la base de datos.
    candidatos = Candidato.objects.filter(
        eleccion=eleccion
    ).select_related('estudiante__usuario').annotate(
        total_votos=Count('votos_recibidos')
    )
    
    # 2. Obtenemos el nombre del candidato a través de la relación con el estudiante y el usuario.
    ...
    context = {
        ...
        'titulo_pagina': f"Análisis de Resultados: {eleccion.nombre}"
    }
...
def gestionar_candidatos_view(request, eleccion_id):
    ...
    candidatos = Candidato.objects.filter(eleccion=eleccion).select_related('estudiante__usuario')
...
def registrar_asistencia_docente_api(request):
    try:
        ...
        docente = Docente.objects.select_related('usuario').get(qr_identifier=qr_identifier)

        # Seguridad multiinstitución
        if not request.user.is_superuser and docente.institucion != request.user.institucion_asociada:
            return JsonResponse({'status': 'error', 'message': 'Docente de otra institución'}, status=403)

        hoy_inicio = timezone.localdate()
        hoy_fin = hoy_inicio + timedelta(days=1)

        # Evitar duplicados por día
        registro_existente = RegistroAsistenciaDocente.objects.filter(
            docente=docente,
        ...
        if registro_existente:
            registro_existente.fecha = timezone.now()
            registro_existente.registrado_por = request.user
            registro_existente.save()
            message = f"Asistencia de {docente.usuario.get_full_name()} actualizada."
        else:
            ...
            message = f"Asistencia de {docente.usuario.get_full_name()} registrada."
...
def asistencias_docentes_hoy_api(request):
    ...
    registros = RegistroAsistenciaDocente.objects.filter(
        fecha__date=hoy,
        institucion=institucion
...
class RegistroAsistenciaDocenteListView(ListView):
    ...
    def get_queryset(self):
        queryset = RegistroAsistenciaDocente.objects.select_related('docente__usuario', 'institucion')
...
def exportar_asistencia_docentes_excel(request):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Asistencia Docentes"

    encabezados = ['Fecha', 'Docente', 'Estado', 'Institución', 'Registrado por']
    ws.append(encabezados)

    for cell in ws[1]:
        cell.font = Font(bold=True)

    for asistencia in queryset.order_by('-fecha'):
        ws.append([
            asistencia.fecha.strftime("%Y-%m-%d %H:%M"),
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = 'attachment; filename=asistencia_docentes.xlsx'
    wb.save(response)
    return response   
...
class ListaAsistenciasDocenteView(ListView):
    ...
    def get_queryset(self):
        ...
        # Si solo quieres mostrarle al docente sus propias asistencias:
        if hasattr(user, 'docente'):
            queryset = queryset.filter(docente=user.docente)

        # Filtrar por institución si manejas multi-institución:
        if hasattr(user, 'institucioneducativa'):
            queryset = queryset.filter(institucion=user.institucioneducativa)
...
def asistente_halu_api(request):
    ...
    try:
        ...
        if not api_key:
            return JsonResponse({'respuesta': "Error de configuración: GOOGLE_API_KEY no definida."}, status=500)
...
@login_required
@never_cache  # <--- 2. AÑADE ESTA LÍNEA JUSTO ANTES DE LA VISTA
def get_planeacion_status_api(request, pk):
    """
    Una vista de API simple que devuelve el estado de generación de una planeación.
    Gracias a @never_cache, esta respuesta nunca será guardada en caché.
    """
    try:
        # Busca la planeación sin filtrar por docente para que un superusuario también pueda verla si es necesario
...
def cursos_por_institucion_api(request, institucion_id):
    # Por seguridad, solo un staff puede usar este filtro
    if not request.user.is_staff:
        return JsonResponse({'error': 'Acceso denegado'}, status=403)
    
    cursos = Curso.objects.filter(institucion_id=institucion_id).select_related('grado', 'materia').order_by('grado__orden', 'materia__nombre_materia')
