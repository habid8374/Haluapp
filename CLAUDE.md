...
- ✅ Si la institución **no** configuró nada, el envío simplemente **no se realiza** (se registra en el log como advertencia). Eso es lo correcto: cada colegio es responsable de configurar su propio canal, y así se le **obliga** a tener su cuenta Brevo/SMTP propia si quiere que sus correos salgan. Su plan/cuota nunca debe verse afectado por otro colegio ni por el sistema.
...
- ❌ **NUNCA** crear variables de entorno nuevas tipo `SISTEMA_BREVO_*`, `BREVO_API_KEY`, `BREVO_SENDER_EMAIL`, etc. como respaldo "global"/"del sistema" pensado para sustituir la credencial de una institución. **No existe ninguna variable de entorno Brevo global en este proyecto** — la credencial vive únicamente en `InstitucionEducativa.brevo_api_key` (y `brevo_sender_email`/`brevo_sender_name`), por institución. Si de verdad no hay institución involucrada (ej. un superusuario de la plataforma sin institución asociada), se usa el `EMAIL_BACKEND` genérico de Django — nunca una cuenta Brevo de ningún tipo.

> Historial: este proyecto SÍ tuvo, por error, un respaldo compartido (`BREVO_API_KEY`/`BREVO_SENDER_EMAIL`/`BREVO_SENDER_NAME` globales en `settings.py`, usado como fallback en `admisiones.utils.enviar_correo_dinamico` y en varias tareas de `admisiones/`, `finanzas/` y `gestion_academica/`). Se eliminó por completo — no se debe reintroducir bajo ninguna forma. `gestion_academica.forms.HaluPasswordResetForm` (correo de "¿Olvidaste tu contraseña?") es el ejemplo de referencia: resuelve la institución del usuario que lo solicita y usa exclusivamente `admisiones.utils.enviar_correo_dinamico` con esa institución.
