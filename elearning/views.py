...
from .services import (
    registrar_cuenta_por_inscripcion_elearning,
    oferta_requiere_pago,
    obtener_cuenta_elearning,
    inscripcion_acceso_contenido_pagado,
)
from gestion_academica.decorators import requiere_pagos_al_dia


def _contexto_pago_elearning(curso, inscripcion=None):
    """Contexto para catálogo/detalle/aula: pago pendiente y enlace Mercado Pago."""
    requiere = oferta_requiere_pago(curso)
    if not inscripcion or not inscripcion.activo:
        return {
            "requiere_pago": requiere,
            "cuenta_pk": None,
            "acceso_pagado": not requiere,
            "inscrito": False,
            "puede_pagar_online": False,
            "deuda_sin_cuenta": False,
        }
    cuenta = obtener_cuenta_elearning(inscripcion)
    acceso = inscripcion_acceso_contenido_pagado(inscripcion)
    cuenta_pk = cuenta.pk if cuenta else None
    return {
        "requiere_pago": requiere,
        "cuenta_pk": cuenta_pk,
        "acceso_pagado": acceso,
        "inscrito": True,
        "puede_pagar_online": bool(requiere and cuenta_pk and not acceso),
        "deuda_sin_cuenta": bool(requiere and not cuenta_pk),
    }
...
def catalogo_cursos(request):
    ...
    cursos = list(Curso.objects.filter(institucion=institucion, publicado=True))
    ...
    if estudiante:
        ...
        inscritos_ids = list(
            InscripcionCurso.objects.filter(
                estudiante=estudiante, activo=True, curso_id__in=curso_ids
            ).values_list("curso_id", flat=True)
        )
        inscripciones = {
            i.curso_id: i
            for i in InscripcionCurso.objects.filter(
                estudiante=estudiante, activo=True, curso_id__in=curso_ids
            ).select_related("curso", "cuenta_por_cobrar")
        }
        for c in cursos:
            pago_por_curso[c.id] = _contexto_pago_elearning(c, inscripciones.get(c.id))
    else:
        for c in cursos:
            pago_por_curso[c.id] = _contexto_pago_elearning(c, None)

    for c in cursos:
        c.ctx_pago = pago_por_curso[c.id]

    return render(
        request,
        "elearning/catalogo.html",
        {
            "cursos": cursos,
...
def detalle_curso(request, curso_id):
    ...
    if estudiante:
        ...
        ).select_related("cuenta_por_cobrar").first()

    inscrito = inscripcion is not None
    ctx_pago = _contexto_pago_elearning(curso, inscripcion)
    ...
    return render(
        request,
        "elearning/detalle.html",
        {"curso": curso, "inscrito": inscrito, "ctx_pago": ctx_pago},
    )
...
def aula_virtual(request, curso_id):
    ...
    ctx_pago = _contexto_pago_elearning(inscripcion.curso, inscripcion)
    bloqueo_pago = ctx_pago["requiere_pago"] and not ctx_pago["acceso_pagado"]
    ...
    if not bloqueo_pago:
        for mod in modulos:
            progreso, created = ProgresoModulo.objects.get_or_create(
                inscripcion=inscripcion, modulo=mod
            )
...
def rendir_evaluacion(request, modulo_id):
    ...
    if not inscripcion_acceso_contenido_pagado(inscripcion):
        messages.warning(
            request,
            "Debes saldar la cuenta de esta oferta en Finanzas para rendir evaluaciones.",
        )
    ...
    return render(request, 'elearning/evaluacion.html', {'evaluacion': evaluacion})

# --- VISTAS DE ADMINISTRACIÓN Y REPORTES ---

@login_required
@permission_required('elearning.add_curso', raise_exception=True)
def gestion_cursos_admin(request):
    ...
    return render(request, 'elearning/admin/lista_cursos.html', {'cursos': cursos})
...
def crear_curso(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('elearning:configurar_curso', curso_id=curso.id)
    else:
        form = CursoForm(user=request.user)
    return render(request, 'elearning/admin/formulario_curso.html', {'form': form, 'titulo': 'Crear Curso'})

@login_required
@permission_required('elearning.change_curso', raise_exception=True)
def editar_curso(request, curso_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('elearning:gestion_cursos_admin')
    else:
        form = CursoForm(instance=curso, user=request.user)
    return render(request, 'elearning/admin/formulario_curso.html', {'form': form, 'titulo': 'Editar Curso'})
...
def configurar_curso(request, curso_id):
    ...
    if request.user.is_superuser:
        ...
        curso = get_object_or_404(Curso, pk=curso_id, institucion=request.user.institucion_asociada)
    return render(request, 'elearning/admin/configurar_curso.html', {'curso': curso})
...
def agregar_modulo(request, curso_id):
    ...
    if request.method == 'POST':
        ...
        form = ModuloForm()
    return render(request, 'elearning/admin/formulario_generico.html', {'form': form, 'titulo': f'Agregar Módulo a {curso.nombre}'})
...
def agregar_material(request, modulo_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('elearning:configurar_curso', curso_id=modulo.curso.id)
    else:
        form = MaterialForm()
    return render(request, 'elearning/admin/formulario_generico.html', {'form': form, 'titulo': f'Agregar Material a {modulo.titulo}'})
...
def agregar_evaluacion(request, modulo_id):
    ...
    if request.method == 'POST':
        ...
        form = EvaluacionForm()
    return render(request, 'elearning/admin/formulario_generico.html', {'form': form, 'titulo': f'Crear Evaluación para {modulo.titulo}'})

@login_required
@permission_required('elearning.add_inscripcioncurso', raise_exception=True)
def matricular_estudiante_manual(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            ok_fc, err_fc = registrar_cuenta_por_inscripcion_elearning(inscripcion)
            if not ok_fc and err_fc:
                messages.warning(request, err_fc)
            if created:
                messages.success(
                    request,
                    f"Estudiante {estudiante} matriculado en {curso.nombre}. Se enviará correo de acceso si hay SMTP configurado.",
                )
            else:
    ...
    return render(request, 'elearning/admin/matricular_manual.html', {
...
@login_required
@permission_required('elearning.view_inscripcioncurso', raise_exception=True)
def reporte_estadisticas_cursos(request):
    ...
    stats_cursos = Curso.objects.filter(institucion=institucion).annotate(
        total_alumnos=Count("inscripcioncurso_set")
    )
    ...
    return render(request, 'elearning/admin/reporte_estadisticas.html', {
...
def generar_certificado_pdf(request, curso_id):
    ...
    """
    if not hasattr(request.user, "estudiante"):
        messages.error(request, "Solo estudiantes pueden descargar el certificado.")
        return redirect("elearning:catalogo")
    estudiante = request.user.estudiante
    inscripcion = get_object_or_404(
        InscripcionCurso.objects.select_related("curso", "cuenta_por_cobrar"),
    ...
    if not inscripcion_acceso_contenido_pagado(inscripcion):
        messages.warning(
            request,
            "Debes completar el pago de esta oferta antes de descargar el certificado.",
        )
        return redirect("elearning:aula_virtual", curso_id=curso_id)
    ...
    # Generar PDF
    template_path = 'elearning/pdfs/certificado.html'
    context = {
        'estudiante': estudiante,
        'curso': inscripcion.curso,
        'fecha': timezone.now(),
...
def registrar_estudiante_curso(request):
    ...
    if request.method == 'POST':
        ...
        if Usuario.objects.filter(email=email).exists():
            ...
            return redirect('elearning:registrar_estudiante_curso')
    ...
    cursos = Curso.objects.filter(institucion=institucion_actual)
    return render(request, 'elearning/admin/registrar_estudiante.html', {
        'cursos': cursos,
        'institucion_actual': institucion_actual,
        'instituciones': instituciones,
        'es_superuser': request.user.is_superuser
...
def lista_estudiantes_curso(request, curso_id):
    ...
    return render(request, 'elearning/admin/lista_estudiantes_curso.html', {
        'curso': curso,
