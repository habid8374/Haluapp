...
class Curso(models.Model):
    ...
    concepto_pago_asociado = models.ForeignKey(
        ConceptoPago, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Material(models.Model):
    ...
    enlace = models.URLField(
        null=True,
        blank=True,
        help_text="URL del video de YouTube o enlace externo",
    )
    orden = models.PositiveIntegerField(default=0)
...
class Evaluacion(models.Model):
    ...
    porcentaje_aprobacion = models.PositiveIntegerField(
        default=60, help_text="Puntaje mínimo de 0 a 100 para aprobar"
    )
    intentos_permitidos = models.PositiveIntegerField(
        default=3, help_text="0 para ilimitados"
    )

    class Meta:
        db_table = "cursos_evaluacion"
...
class Opcion(models.Model):
    ...
    class Meta:
        db_table = "cursos_opcion"


class InscripcionCurso(models.Model):
    estudiante = models.ForeignKey(
        Estudiante, related_name="elearning_inscripciones", on_delete=models.CASCADE
    )
...
class ProgresoModulo(models.Model):
    inscripcion = models.ForeignKey(
        InscripcionCurso, related_name="progresos_modulos", on_delete=models.CASCADE
    )
